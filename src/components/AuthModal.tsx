...
export const AuthModal: React.FC<AuthModalProps> = ({ onSuccess }) => {
  ...
  const handleCopy = (text: string, isDomain: boolean) => {
  ...
  const handleGuestSignIn = async () => {
    ...
    try {
      ...
      onSuccess?.(`Signed in as Guest Attendee! You can now reserve and view your pass.`);
      closeAuthModal();
    } catch (err: any) {
      const { simpleMsg, info } = parseFirebaseError(err);
      setError(simpleMsg);
    ...
    >
      ...
      >
        ...
        <div className="p-6 pt-5 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              {errorInfo.type === 'unauthorized-domain' && currentHost && (
                <div className="mt-2 pt-2 border-t border-amber-200/80 space-y-2">
                  ...
                  <div className="flex items-center gap-2 bg-white/90 p-2 rounded-lg border border-amber-200 font-mono text-[11px] text-gray-800 break-all">
                    ...
                    <button
                      type="button"
                      onClick={() => handleCopy(currentHost, true)}
                  ...
                  <p className="text-[11px] text-amber-700 italic pt-1">
                    Tip: You can also use <strong>Email & Password</strong> or <strong>Guest Mode</strong> below right now without waiting for domain authorization.
                  </p>
                </div>
              )}
          ...
          {/* Quick Guest Attendee Access (Zero friction testing) */}
          {!isForgotMode && (
            <div className="pt-2 border-t border-gray-100 flex flex-col items-center gap-2">
              <button
                ...
                disabled={loading || googleLoading || guestLoading}
                className="w-full py-2 px-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl text-xs font-semibold text-gray-700 hover:text-gray-900 transition-colors flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
              >
                {guestLoading ? (
                ...
                <span>Continue as Guest Attendee (Instant Access)</span>
              </button>
            </div>
          )}

          {/* Expandable Firebase Configuration & Diagnostics Guide */}
          <div className="pt-2 border-t border-gray-100">
            <button
              type="button"
              onClick={() => setShowConfigHelp(!showConfigHelp)}
              className="w-full flex items-center justify-between text-xs text-gray-500 hover:text-blue-700 py-1 transition-colors cursor-pointer"
            >
              <span className="flex items-center gap-1.5 font-medium">
                <HelpCircle className="w-3.5 h-3.5 text-blue-600" />
                Firebase Authorization & Setup Guide
              </span>
              <span className="text-[11px] font-mono text-gray-400">
                {showConfigHelp ? 'Hide ▲' : 'Show ▼'}
              </span>
            </button>

            {showConfigHelp && (
              <div className="mt-3 p-3.5 rounded-xl bg-gray-50 border border-gray-200 text-xs text-gray-700 space-y-3 animate-in fade-in">
                <div>
                  <span className="font-semibold text-gray-900 block mb-0.5">1. Authorized Domains:</span>
                  <p className="text-[11px] text-gray-600 mb-1.5">
                    For Google Sign-In, add your current domain to Firebase Console &gt; Authentication &gt; Settings &gt; Authorized domains:
                  </p>
                  <div className="flex items-center gap-1.5 bg-white p-1.5 rounded-lg border border-gray-200 font-mono text-[11px]">
                    <span className="truncate flex-1">{currentHost}</span>
                    <button
                      type="button"
                      onClick={() => handleCopy(currentHost, true)}
                      className="px-2 py-0.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded text-[10px] font-sans font-medium flex items-center gap-1 shrink-0 cursor-pointer"
                    >
                ...
                <div>
                  <span className="font-semibold text-gray-900 block mb-0.5">2. Sign-in Providers:</span>
                  <p className="text-[11px] text-gray-600">
                    Enable <strong>Email/Password</strong>, <strong>Google</strong>, and optionally <strong>Anonymous</strong> in Firebase Console &gt; Authentication &gt; Sign-in method.
                  </p>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      type="button"
                      onClick={() => handleCopy(recommendedRules, false)}
                      className="text-[10px] text-blue-700 hover:underline flex items-center gap-1 font-medium cursor-pointer"
                    >
                      {copiedRules ? <Check className="w-3 h-3 text-green-600" /> : <Copy className="w-3 h-3" />}
                      <span>{copiedRules ? 'Copied Rules' : 'Copy Rules'}</span>
                    </button>
                  </div>
                  <pre className="p-2 bg-gray-900 text-gray-100 rounded-lg text-[10px] font-mono overflow-x-auto leading-tight">
                    {recommendedRules}
                  </pre>
                </div>
              </div>
          ...
          <div className="pt-1 text-center text-[11px] text-gray-400 flex items-center justify-center gap-1.5">
