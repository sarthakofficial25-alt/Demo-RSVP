...
export const Footer: React.FC<FooterProps> = ({ onNavClick, onOpenAdminRsvps }) => {
  return (
        ...
        <div className="mt-8 pt-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between text-xs text-gray-600 gap-2">
          ...
          <div className="flex items-center gap-3">
            {onOpenAdminRsvps && (
              <button
                type="button"
                onClick={onOpenAdminRsvps}
                className="font-mono text-[11px] text-blue-600 hover:text-blue-800 hover:underline cursor-pointer bg-blue-50 px-2 py-0.5 rounded border border-blue-200"
              >
                Firestore RSVPs & Sync
              </button>
            )}
